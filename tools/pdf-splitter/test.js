...
const testGetPages = (str, max) => Array.from(getPagesToExtract(str, max)).sort((a, b) => a - b);

// Test cases
try {
    // 1. Empty range should return all pages
    assert.deepStrictEqual(testGetPages("", 5), [0, 1, 2, 3, 4], "Empty range failed");
    assert.deepStrictEqual(testGetPages("   ", 5), [0, 1, 2, 3, 4], "Whitespace range failed");

    // 2. Valid single pages
    assert.deepStrictEqual(testGetPages("1, 3, 5", 5), [0, 2, 4], "Single pages failed");
    assert.deepStrictEqual(testGetPages(" 1 , 3 , 5 ", 5), [0, 2, 4], "Single pages with whitespace failed");

    // 3. Valid ranges
    assert.deepStrictEqual(testGetPages("1-3", 5), [0, 1, 2], "Simple range failed");
    assert.deepStrictEqual(testGetPages("3-1", 5), [0, 1, 2], "Reverse range failed");
    assert.deepStrictEqual(testGetPages("1-3, 5", 5), [0, 1, 2, 4], "Mixed range and single failed");

    // 4. Overlapping ranges
    assert.deepStrictEqual(testGetPages("1-4, 3-5", 5), [0, 1, 2, 3, 4], "Overlapping ranges failed");
    assert.deepStrictEqual(testGetPages("1-2, 1-2", 5), [0, 1], "Duplicate ranges failed");
    assert.deepStrictEqual(testGetPages("1, 1, 1", 5), [0], "Duplicate singles failed");

    // 5. Out of bounds
    assert.deepStrictEqual(testGetPages("1-10", 5), [0, 1, 2, 3, 4], "Upper out of bounds range failed");
    assert.deepStrictEqual(testGetPages("-5", 5), [], "Lower out of bounds (negative) failed");
    assert.deepStrictEqual(testGetPages("0", 5), [], "Page 0 failed");
    assert.deepStrictEqual(testGetPages("6", 5), [], "Page > max failed");

    // 6. Malformed strings
    assert.deepStrictEqual(testGetPages("abc", 5), [], "Alphabetic string failed");
    assert.deepStrictEqual(testGetPages("1-abc", 5), [], "Half malformed range failed");
    assert.deepStrictEqual(testGetPages("1-3, abc", 5), [0, 1, 2], "Mixed valid and malformed failed");
